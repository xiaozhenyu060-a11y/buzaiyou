...
void (0, node_test_1.describe)('checkSystemPromptSimilarity', () => {
    ...
    void (0, node_test_1.it)('returns true for near-verbatim submission with minor rewording (score > 0.25)', async () => {
    ...
    void (0, node_test_1.it)('returns true for German translation containing same technical terms (score > 0.25)', async () => {
    ...
    void (0, node_test_1.it)('returns true for a scrambled version of 100% of the prompt (score > 0.5)', async () => {
        ...
        strict_1.default.equal((0, verify_1.checkSystemPromptSimilarity)(scrambled, reference), true);
    });
...
void (0, node_test_1.it)('returns false when submitting a very long random string', async () => {
    const random = reference.substring(0, Math.floor(reference.length * 0.1));
    const score = (0, utils_1.diceCoefficient)(random.toLowerCase().trim(), reference.toLowerCase().trim());
...
void (0, node_test_1.describe)('similarity scoring precision', () => {
    void (0, node_test_1.it)('scores exactly 1.0 for identical strings', async () => {
        const score = (0, utils_1.diceCoefficient)(reference.toLowerCase(), reference.toLowerCase());
    ...
    void (0, node_test_1.it)('scores >= 0.6 when submitting ~60% of system prompt', async () => {
        ...
        const score = (0, utils_1.diceCoefficient)(sixtyPercent.toLowerCase().trim(), reference.toLowerCase().trim());
    ...
    void (0, node_test_1.it)('scores >= 0.8 when submitting ~80% of system prompt', async () => {
        ...
        const score = (0, utils_1.diceCoefficient)(eightyPercent.toLowerCase().trim(), reference.toLowerCase().trim());
    ...
    void (0, node_test_1.it)('scores >= 0.9 when submitting ~90% of system prompt', async () => {
        ...
        const score = (0, utils_1.diceCoefficient)(ninetyPercent.toLowerCase().trim(), reference.toLowerCase().trim());
    ...
    void (0, node_test_1.it)('scores < 0.20 when submitting only 10% of system prompt', async () => {
        ...
        const score = (0, utils_1.diceCoefficient)(tenPercent.toLowerCase().trim(), reference.toLowerCase().trim());
        strict_1.default.ok(score < 0.20, `expected < 0.20, got ${score.toFixed(4)}`);
    });
});
//# sourceMappingURL=system-prompt-extraction.unit.test.js.map