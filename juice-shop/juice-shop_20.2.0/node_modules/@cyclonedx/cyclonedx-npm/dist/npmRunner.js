...
class NpmRunner {
    static #jsMatcher = /\.[cm]?js$/;
    static #npxMatcher = /(^|\\|\/)npx-cli\.js$/;
    constructor(process_, console_) {
    ...
    static #getExecPath(process_, console_) {
        ...
        if (NpmRunner.#npxMatcher.test(execPath)) {
            ...
            const npmPath = (0, node_path_1.resolve)(execPath.replace(NpmRunner.#npxMatcher, '$1npm-cli.js'));
            if ((0, node_fs_1.existsSync)(npmPath)) {
                return npmPath;
            }
        }
        else if ((0, node_fs_1.existsSync)(execPath)) {
            return execPath;
        }
        throw new Error(`unexpected NPM execPath: ${execPath}`);
    }
    static #makeNpmRunner(process_, console_) {
        ...
        if (execPath === undefined) {
            ...
            return (args, options) => (0, node_child_process_1.execSync)('npm ' + args.join(' '), options);
        }
        if (NpmRunner.#jsMatcher.test(execPath)) {
            ...
            return (args, options) => (0, node_child_process_1.execFileSync)(nodeExecPath, ['--', execPath, ...args], options);
        }
        console_.debug('DEBUG | makeNpmRunner caused execFileSync "%s"', execPath);
        return (args, options) => (0, node_child_process_1.execFileSync)(execPath, args, options);
    }
}
