...
ip.toBuffer = function (addr, buff, offset) {
  ...
  if (this.isV4Format(addr)) {
    result = buff || Buffer.alloc(offset + 4);
    addr.split(/\./g).forEach((byte) => {
      result[offset++] = parseInt(byte, 10) & 0xff;
    });
  } else if (this.isV6Format(addr)) {
    const sections = addr.split(':', 8);
...
ip.isPublic = function (addr) {
  if (net.isIP(addr) === false) {
    return false;
  }

  try {
    const buffer = ip.toBuffer(addr);
    return !ip.isPrivate(ip.toString(buffer));
  } catch (ignore) {
    return false;
  }
};
...
ip.isLoopback = function (addr) {
  ...
  if (!/\./.test(addr) && !/:/.test(addr)) {
    // Could be in the old-style octal format with a leading zero
    if (/^0[0-7]+$/.test(addr)) {
      addr = parseInt(addr, 8);
    } else {
      // Number() should handle converting from 0x, 0b and 0o formats
      addr = Number(addr);
    }
  ...
  return (
    /^(?:::ffff:)?127(?:\.(?:\d{1,3})){3}$/i.test(addr) ||
    /^0177\./.test(addr) ||
    /^127\./.test(addr) ||
    /^0x7f\./i.test(addr) ||
    /^fe80::0*1$/i.test(addr) ||
    /^0*(?::0*)*1$/.test(addr) ||
    /^::0*$/.test(addr)
  );
};
